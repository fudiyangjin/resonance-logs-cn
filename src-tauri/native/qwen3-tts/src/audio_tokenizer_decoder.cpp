...
void AudioTokenizerDecoder::normalize_codebooks() {
    ...
    // NOTE: codebook/usage tensors live in whatever buffer
    // load_tensor_data_from_file allocated them in -- the backend selected
    // by init_preferred_backend, which is a Vulkan device buffer on Vulkan
    // builds. `tensor->data` is only a directly-dereferenceable host
    // pointer for CPU-backend tensors; for Vulkan (and other non-host
    // backends) it's backend-internal and reading/writing it directly from
    // host code is a silent access violation. Always go through
    // ggml_backend_tensor_get/set, which know how to do a real device
    // transfer when needed and are a no-op copy for host buffers.
    auto normalize_codebook = [epsilon](struct ggml_tensor * codebook, struct ggml_tensor * usage) {
        ...
        ggml_backend_tensor_get(codebook, cb_data.data(), 0, cb_data.size() * sizeof(ggml_fp16_t));
        ggml_backend_tensor_get(usage, usage_data.data(), 0, usage_data.size() * sizeof(float));
        ...
        ggml_backend_tensor_set(codebook, cb_data.data(), 0, cb_data.size() * sizeof(ggml_fp16_t));
    };
