...
static bool get_process_memory_snapshot(process_memory_snapshot & out) {
    ...
    if (GetProcessMemoryInfo(GetCurrentProcess(), &stats, sizeof(stats)) == 0) {
        return false;
    }
...
bool Qwen3TTS::load_models(const std::string & tts_model_path,
...
tts_result Qwen3TTS::synthesize(const std::string & text,
    ...
    return synthesize_internal(text, zero_embedding.data(), -1, params, result);
}
...
tts_result Qwen3TTS::synthesize_with_voice(const std::string & text,
    ...
    return synthesize_internal(text, speaker_embedding.data(), -1, params, result);
}
...
tts_result Qwen3TTS::synthesize_with_embedding(const std::string & text,
    ...
    // The transformer reads exactly hidden_size floats from this pointer
    // with no bounds check of its own, so a stale/corrupted cached
    // embedding (e.g. extracted with a different model) must be rejected
    // here rather than risk an out-of-bounds read during generation.
    const int32_t expected_size = transformer_.get_config().hidden_size;
    if (embedding_size != expected_size) {
        result.error_msg = "Speaker embedding size mismatch: got " + std::to_string(embedding_size) +
                            ", model expects " + std::to_string(expected_size) +
                            " (embedding was likely extracted with a different model)";
        return result;
    }

    return synthesize_internal(text, embedding, -1, params, result);
}

tts_result Qwen3TTS::synthesize_with_speaker_token(const std::string & text,
    ...
    if (speaker_token_id < 0 || speaker_token_id >= transformer_.get_config().codec_vocab_size) {
        result.error_msg = "Speaker token ID is outside the codec embedding vocabulary";
        return result;
    }
    return synthesize_internal(text, nullptr, speaker_token_id, params, result);
}
...
int32_t Qwen3TTS::get_codec_vocab_size() const {
    return models_loaded_ ? transformer_.get_config().codec_vocab_size : 0;
}
...
tts_result Qwen3TTS::synthesize_internal(const std::string & text,
    ...
    if (!transformer_.generate(text_tokens.data(), (int32_t)text_tokens.size(),
                               ...
                               params.temperature, params.top_k, speaker_token_id)) {
