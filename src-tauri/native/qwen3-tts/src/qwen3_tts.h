...
struct tts_params {
    ...
    // Number of threads. Currently unused: CPU backends get their thread
    // count from gguf_loader's apply_default_cpu_threads() (based on
    // hardware_concurrency) at backend-creation time, not per generation
    // call, since ggml sets thread count on the backend, not per-graph.
    int32_t n_threads = 4;
...
public:
    ...
    // Load a transformer and shared audio tokenizer from explicit paths.
    bool load_models(const std::string & tts_model_path,
    ...
    // embedding_size: must match hidden_size (typically 1024). Returns a
    // failed result (see get_error()) instead of touching the embedding
    // buffer if the size does not match, so a corrupted/incompatible
    // cached embedding can never cause an out-of-bounds read.
    tts_result synthesize_with_embedding(const std::string & text,
                                          const float * embedding, int32_t embedding_size,
                                          const tts_params & params = tts_params());

    // Synthesize with a learned CustomVoice speaker row stored in codec_embd.
    tts_result synthesize_with_speaker_token(const std::string & text,
                                              int32_t speaker_token_id,
                                              const tts_params & params = tts_params());

    // Hidden size (speaker embedding dimensionality) of the loaded
    // transformer, or 0 if no model is loaded yet.
    int32_t get_hidden_size() const;
    int32_t get_codec_vocab_size() const;
