...
Qwen3Tts * qwen3_tts_create(const char * model_dir, int32_t n_threads) {
    ...
    // NOTE: this explicit override isn't wired through yet -- Qwen3TTS
    // doesn't expose its underlying ggml_backend_t(s) for a caller to
    // reconfigure after load_models(). It's not a functional regression
    // though: load_models() ends up going through gguf_loader's shared
    // backend singleton, which now applies a hardware_concurrency-based
    // thread count on creation instead of the old hardcoded default of 4
    // (see apply_default_cpu_threads in gguf_loader.cpp).
    (void)n_threads;
