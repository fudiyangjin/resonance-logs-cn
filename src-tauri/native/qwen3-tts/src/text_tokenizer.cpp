...
// Generate the reversible GPT-2 byte encoder instead of relying on the
// legacy literal table above, whose non-ASCII entries were encoding-damaged.
static std::array<std::string, 256> build_gpt2_byte_to_unicode() {
    std::array<std::string, 256> result;
    uint32_t escaped_codepoint = 256;
    for (uint32_t byte = 0; byte < 256; ++byte) {
        const bool printable = (byte >= 33 && byte <= 126)
            || (byte >= 161 && byte <= 172)
            || (byte >= 174);
        result[byte] = codepoint_to_utf8(printable ? byte : escaped_codepoint++);
    }
    return result;
}

static const std::array<std::string, 256> GPT2_BYTE_TO_UNICODE =
    build_gpt2_byte_to_unicode();
...
static std::unordered_map<std::string, uint8_t> build_unicode_to_byte() {
    ...
    for (int i = 0; i < 256; i++) {
        result[GPT2_BYTE_TO_UNICODE[i]] = (uint8_t)i;
    }
    return result;
}
        ...
        for (const auto & tok : bpe_tokens) {
            ...
            if (it != vocab_.end()) {
                ...
                for (unsigned char c : tok) {
                    std::string byte_tok = GPT2_BYTE_TO_UNICODE[c];
