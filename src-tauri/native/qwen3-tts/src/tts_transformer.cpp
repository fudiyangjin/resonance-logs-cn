...
// Historically every `ffn_down` mul_mat pre-cast the weight tensor to F32
// (see upstream commit 8783e53, "Fix F16 precision: cast ffn_down weights
// to F32 in all 5 graph builders"). That fix targeted plain-F16 model
// weights, where some GGML mul_mat paths lose precision versus a full F32
// matmul. Block-quantized weights (Q8_0, Q4_K, ...) don't have that
// problem: ggml's quantized mul_mat kernels already dequantize each block
// to full float precision internally before accumulating, which is exactly
// what `ggml_cast` to F32 would do -- just fused into the matmul instead of
// materializing a full-size intermediate tensor first. Casting a quantized
// tensor here therefore buys no accuracy and only costs a full dequant of
// the whole weight on every call (extra memory bandwidth on CPU, and on
// Vulkan it forces the generic F32 matmul shader instead of the
// quantized-weight kernel already used for the sibling ffn_gate/ffn_up
// weights in the same layer). Keep the F32 cast only for the case it was
// actually written for.
static struct ggml_tensor * ffn_down_mul_mat_weight(
    struct ggml_context * ctx0, struct ggml_tensor * ffn_down) {
    if (ggml_is_quantized(ffn_down->type)) {
        return ffn_down;
    }
    return ggml_cast(ctx0, ffn_down, GGML_TYPE_F32);
}
...
bool TTSTransformer::load_model(const std::string & model_path) {
    ...
    if (device && ggml_backend_dev_type(device) != GGML_BACKEND_DEVICE_TYPE_CPU) {
        ...
        apply_default_cpu_threads(state_.backend_cpu);
    }
...
bool TTSTransformer::build_prefill_graph(const int32_t * text_tokens, int32_t n_tokens,
    ...
    if (speaker_token_id >= 0) {
        ...
        if (!lookup_single_embedding_row(model_.codec_embd, speaker_token_id,
                                         speaker_token_embed.data())) {
            return false;
        }
    ...
    // CustomVoice uses the official Python API's non-streaming mode by
    // default. Prefill the complete text so language and pronunciation are
    // conditioned on the whole utterance instead of revealing one text token
    // per generated audio frame.
    if (speaker_token_id >= 0) {
        const int32_t content_token_count = n_tokens - 8;
        if (content_token_count <= 0) {
            error_msg_ = "CustomVoice prompt does not contain text tokens";
            return false;
        }

        std::vector<float> content_text_proj;
        if (!project_text_tokens(text_tokens + 3, content_token_count, content_text_proj)) {
            return false;
        }

        const float * codec_pad_embed =
            codec_input_embedding.data() + (size_t)(codec_input_len - 2) * hidden_size;
        const float * codec_bos_embed =
            codec_input_embedding.data() + (size_t)(codec_input_len - 1) * hidden_size;
        const int32_t prefix_len = 3 + codec_plus_overlay_len;
        const int32_t prefill_len = prefix_len + content_token_count + 2;
        ...
        for (int32_t t = 0; t < content_token_count; ++t) {
            const float * text_row = content_text_proj.data() + (size_t)t * hidden_size;
            float * out_row = prefill_embd.data() + (size_t)(prefix_len + t) * hidden_size;
            for (int32_t h = 0; h < hidden_size; ++h) {
                out_row[h] = text_row[h] + codec_pad_embed[h];
            }
        }

        float * eos_row = prefill_embd.data()
            + (size_t)(prefix_len + content_token_count) * hidden_size;
        float * bos_row = eos_row + hidden_size;
        for (int32_t h = 0; h < hidden_size; ++h) {
...
struct ggml_cgraph * TTSTransformer::build_prefill_forward_graph(int32_t n_tokens, int32_t n_past) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        cur = ggml_mul_mat(ctx0, ffn_down_mul_mat_weight(ctx0, layer.ffn_down), cur);
...
struct ggml_cgraph * TTSTransformer::build_step_graph(int32_t n_past, bool fused_embed) {
    const auto & cfg = model_.config;
    const int n_head = cfg.n_attention_heads;
    const int n_kv_head = cfg.n_key_value_heads;
    const int head_dim = cfg.head_dim;
    ...
    if (fused_embed) {
        // Build [codec_embd(codes[0]) + sum_{cb=1..n_codebooks-1} code_pred_embd[cb-1](codes[cb])
        // + trailing_row] entirely on-device: ggml_get_rows replaces what
        // the caller would otherwise do via n_codebooks separate
        // lookup_single_embedding_row() backend->host reads (each a full
        // round trip on non-CPU backends), and ggml_add replaces the
        // host-side summation.
        const int32_t n_codebooks = cfg.n_codebooks;
        struct ggml_tensor * inp_codes = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_codebooks);
        ggml_set_name(inp_codes, "inp_codes");
        ggml_set_input(inp_codes);

        struct ggml_tensor * inp_trailing = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, hidden_size, 1);
        ggml_set_name(inp_trailing, "inp_trailing");
        ggml_set_input(inp_trailing);

        auto embed_row_f32 = [&](struct ggml_tensor * table, int32_t codebook_idx) {
            struct ggml_tensor * id = ggml_view_1d(ctx0, inp_codes, 1, (size_t)codebook_idx * sizeof(int32_t));
            struct ggml_tensor * row = ggml_get_rows(ctx0, table, id);
            if (row->type != GGML_TYPE_F32) {
                row = ggml_cast(ctx0, row, GGML_TYPE_F32);
            }
            return row;
        };

        struct ggml_tensor * sum = embed_row_f32(model_.codec_embd, 0);
        for (int32_t cb = 1; cb < n_codebooks; ++cb) {
            sum = ggml_add(ctx0, sum, embed_row_f32(model_.code_pred_embd[cb - 1], cb));
        }
        cur = ggml_add(ctx0, sum, inp_trailing);
    } else {
...
bool TTSTransformer::forward_step_fused(const int32_t * frame_codes, const float * trailing_row,
                                        int32_t n_past, std::vector<float> & output) {
    if (!model_.ctx) {
        error_msg_ = "Model not loaded";
        return false;
    }
    if (!frame_codes || !trailing_row) {
        error_msg_ = "frame_codes/trailing_row is null";
        return false;
    }
    ...
    t0 = clk::now();
#endif
    struct ggml_cgraph * gf = build_step_graph(n_past, /*fused_embed=*/true);
#ifdef QWEN3_TTS_TIMING
    t1 = clk::now();
    if (timing_) timing_->t_talker_graph_build_ms += std::chrono::duration<double, std::milli>(t1 - t0).count();
    t0 = clk::now();
    ...
    if (inp_codes) {
        ggml_backend_tensor_set(inp_codes, frame_codes, 0,
                                model_.config.n_codebooks * sizeof(int32_t));
    }

    struct ggml_tensor * inp_trailing = ggml_graph_get_tensor(gf, "inp_trailing");
    if (inp_trailing) {
        ggml_backend_tensor_set(inp_trailing, trailing_row, 0,
                                model_.config.hidden_size * sizeof(float));
    }
...
bool TTSTransformer::generate(const int32_t * text_tokens, int32_t n_tokens,
    ...
    for (int frame = 0; frame < max_len; ++frame) {
        ...
        // frame_codes[0..n_codebooks-1] + the trailing row are summed
        // on-device by forward_step_fused's fused_embed graph (get_rows +
        // add), instead of doing n_codebooks separate embedding-table
        // reads back to the host and summing them here.
        ...
        t0 = clk::now();
#endif
        if (!forward_step_fused(frame_codes.data(), trailing_row, n_past, logits)) {
            return false;
        }
#ifdef QWEN3_TTS_TIMING
        t1 = clk::now();
