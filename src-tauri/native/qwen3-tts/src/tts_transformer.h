...
struct tts_timing {
    ...
    // Embed lookups in generate() loop. Always 0 now that the per-frame
    // embedding assembly is fused into forward_step_fused's graph (folded
    // into t_talker_forward_ms/t_talker_compute_ms instead); kept as a
    // field so older bench parsers relying on this key don't break.
...
public:
    ...
    // Same as forward_step, but assembles the per-frame input embedding
    // (codec_embd(frame_codes[0]) + sum of code_pred_embd(frame_codes[1..])
    // + trailing_row) inside the compute graph via ggml_get_rows/ggml_add
    // instead of the caller doing per-row backend->host reads and a
    // host-side sum. Avoids ~n_codebooks device->host round trips per
    // frame on non-CPU backends (see build_step_graph's fused_embed path).
    // frame_codes must have model_.config.n_codebooks entries.
    bool forward_step_fused(const int32_t * frame_codes, const float * trailing_row,
                            int32_t n_past, std::vector<float> & output);
...
private:
    ...
    struct ggml_cgraph * build_step_graph(int32_t n_past, bool fused_embed = false);
