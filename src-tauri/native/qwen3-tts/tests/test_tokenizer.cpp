...
void print_usage(const char * prog) {
    printf("Usage: %s --model <path_to_gguf> [--text <text>]\n", prog);
    printf("       %s --model <path_to_gguf> --text-file <utf8_file>\n", prog);
...
int main(int argc, char ** argv) {
    ...
    const char * custom_text = nullptr;
    const char * custom_text_file = nullptr;
    ...
    std::string custom_text_storage;
    if (custom_text_file) {
        std::ifstream input(custom_text_file, std::ios::binary);
        custom_text_storage.assign(
            std::istreambuf_iterator<char>(input),
            std::istreambuf_iterator<char>());
        while (!custom_text_storage.empty()
               && (custom_text_storage.back() == '\n' || custom_text_storage.back() == '\r')) {
            custom_text_storage.pop_back();
        }
        custom_text = custom_text_storage.c_str();
    }
    if (custom_text) {
        auto custom_tokens = tokenizer.encode_for_tts(custom_text);
        printf("Custom TTS tokens: [");
        for (size_t i = 0; i < custom_tokens.size(); ++i) {
            printf("%d%s", custom_tokens[i], i + 1 < custom_tokens.size() ? ", " : "");
        }
        printf("]\n");
        return 0;
    }
