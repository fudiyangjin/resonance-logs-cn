...
static void print_usage(const char * prog) {
    ...
    printf("  --language-id <n>    Codec language token ID (default: 2050)\n");
    printf("  --speaker-token <n>  CustomVoice speaker token ID (default: disabled)\n");
...
int main(int argc, char ** argv) {
    ...
    int max_len = 64;
    int language_id = 2050;
    int speaker_token_id = -1;
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--model") == 0 && i + 1 < argc) {
            ...
            max_len = atoi(argv[++i]);
        } else if (strcmp(argv[i], "--language-id") == 0 && i + 1 < argc) {
            language_id = atoi(argv[++i]);
        } else if (strcmp(argv[i], "--speaker-token") == 0 && i + 1 < argc) {
            speaker_token_id = atoi(argv[++i]);
    ...
    const float * spk_ptr = speaker_token_id >= 0 || speaker_embd.empty()
        ? nullptr
        : speaker_embd.data();

    std::vector<int32_t> generated_codes;
    printf("  Calling generate(n_tokens=%d, max_len=%d, language_id=%d, speaker_token_id=%d)...\n",
           n_tokens, max_len, language_id, speaker_token_id);

    bool gen_ok = transformer.generate(
        text_tokens.data(), n_tokens, spk_ptr, max_len,
        generated_codes, language_id, 1.05f, 0.0f, 0, speaker_token_id);
