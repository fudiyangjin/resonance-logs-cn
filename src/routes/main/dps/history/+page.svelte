	...
	async function confirmDeleteSelected() {
		...
		try {
			...
			if (res.status === "ok") {
				...
				errorMsg = `Delete failed: ${res.error}`;
			}
		} catch (e) {
...
<div class="">
		...
		{#if hasActiveFilters}
			<div class="flex flex-wrap items-center gap-1.5">
				...
				{#each selectedBosses as boss}
					...
					>
						<span class="text-muted-foreground/70">Boss: </span>
						{boss}
						<button
				...
				{#each selectedPlayerNames as player}
					<span
						class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-popover text-muted-foreground leading-tight border border-border/60"
					>
						<span class="text-muted-foreground/70">Player: </span>
						{player}
						<button
						...
						<td class="px-3 py-2 text-sm text-muted-foreground">
							<button
								...
								class="flex items-center justify-center w-5 h-5 rounded border transition-colors {selectedIds.has(enc.id)
									? 'bg-primary border-primary'
									: 'border-border hover:border-primary/50'}"
								aria-label={selectedIds.has(enc.id) ? "Deselect" : "Select"}
							>
	...
	<div class="flex items-center justify-between mt-4 gap-4">
		...
		<div class="flex items-center gap-1 ml-auto">
			<button onclick={() => loadEncounters(0)} disabled={page === 0} class="p-1.5 text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed transition-colors" aria-label="First page">
				<svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
				</svg>
			</button>
			<button onclick={() => loadEncounters(page - 1)} disabled={page === 0} class="p-1.5 text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed transition-colors" aria-label="Previous page">
				<svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
				</svg>
			</button>
			<button onclick={() => loadEncounters(page + 1)} disabled={(page + 1) * pageSize >= totalCount} class="p-1.5 text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed transition-colors" aria-label="Next page">
				<svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
				</svg>
			</button>
			<button onclick={() => loadEncounters(Math.floor((totalCount - 1) / pageSize))} disabled={(page + 1) * pageSize >= totalCount} class="p-1.5 text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed transition-colors" aria-label="Last page">
				<svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	<div class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 duration-200">
		<div class="flex items-center gap-4 px-5 py-3 rounded-xl border border-border bg-popover/95 backdrop-blur-sm shadow-xl">
			<div class="flex items-center gap-2 text-sm">
				...
				<span class="text-muted-foreground">records selected</span>
			</div>
	...
	>
		...
		<div class="relative z-10 w-full max-w-md mx-4 p-6 rounded-xl border border-border bg-popover shadow-2xl animate-in fade-in zoom-in-95 duration-200">
			...
			<p class="text-sm text-muted-foreground mb-6">
				Are you sure you want to permanently delete {selectedIds.size === 1 ? 'this encounter record' : 'these encounter records'}? All associated data (player stats, skill stats, death events, etc.) will be removed.
			</p>
