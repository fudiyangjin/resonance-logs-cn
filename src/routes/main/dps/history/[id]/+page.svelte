<script lang="ts">
  ...
  function flattenGrouping(grouping: {
    ...
    for (const group of grouping.groups) {
      rows.push({ kind: "group", key: `g-${group.recountId}`, depth: 0, row: group });
      if (!expandedGroups.has(group.recountId)) continue;
      for (const skill of group.skills) {
        rows.push({ kind: "skill", key: `gs-${group.recountId}-${skill.skillId}`, depth: 1, row: skill });
      }
    }
    for (const skill of grouping.ungrouped) {
      rows.push({ kind: "skill", key: `u-${skill.skillId}`, depth: 0, row: skill });
    }
    return rows;
  }
  ...
  let healTargetTotal = $derived.by(() =>
    healTargetSummary.reduce((sum, target) => sum + target.totalValue, 0)
  );

  function rowTotalDmg(row: FlatSkillRow): number { return row.row.totalDmg ?? 0; }
  function rowDmgPct(row: FlatSkillRow): number { return row.row.dmgPct ?? 0; }
  ...
  const websiteBaseUrl = $derived.by(() => {
    ...
    if (!apiBase) return "https://bpsr.app";
    try {
      const url = new URL(apiBase);
      if (url.hostname.startsWith("api.")) url.hostname = url.hostname.replace(/^api\./, "");
      url.pathname = "";
  ...
  async function loadEncounter() {
    ...
    try {
      ...
      if (encounterRes.status !== "ok") { error = String(encounterRes.error); return; }
      if (entitiesRes.status !== "ok") { error = String(entitiesRes.error); return; }
      encounter = encounterRes.data;
      localPlayerUid = (encounterRes.data as { localPlayerId?: number | null }).localPlayerId ?? null;
      rawEntities = entitiesRes.data;
      const durationSeconds =
        encounterRes.data.duration > 0
          ? Math.max(1, encounterRes.data.duration)
          : Math.max(1, ((encounterRes.data.endedAtMs ?? Date.now()) - encounterRes.data.startedAtMs) / 1000);
      players = buildHistoryPlayers(rawEntities, durationSeconds, encounterRes.data.activeCombatDuration ?? null, localPlayerUid);
    } catch (err) {
  ...
  function viewPlayerSkills(playerUid: number, type = "dps", targetUid?: number | null) {
    ...
    if (type === "dps" && targetUid != null) { sp.set("targetUid", String(targetUid)); } else { sp.delete("targetUid"); }
    goto(`/main/dps/history/${encounterId}?${sp.toString()}`);
  }

  function backToEncounter() {
    const sp = new URLSearchParams($page.url.searchParams);
    sp.delete("charId"); sp.delete("skillType"); sp.delete("targetUid");
  ...
  async function openEncounterOnWebsite() {
    ...
    try { await openUrl(url); } catch (err) { console.error("Failed to open URL:", url, err); }
  }

  $effect(() => { loadEncounter(); });
  $effect(() => { charId; expandedGroups = new Set<number>(); });
  $effect(() => { activeTab; if (activeTab !== "damage") { overviewTargetUid = null; } });
        ...
        <div class="flex flex-wrap items-stretch justify-between gap-3">
          <div class="flex items-start gap-3 min-w-0 flex-1">
            <div class="space-y-1 min-w-0 flex-1 h-full">
              ...
              {/if}
              <div class="flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
                <span>{new Date(encounter.startedAtMs).toLocaleString("en-CA")}</span>
          ...
          <div class="flex flex-col items-end gap-2 shrink-0 self-stretch justify-between h-full">
            <div class="flex items-center gap-1.5">
              ...
              <button
                ...
                title={encounter.isFavorite ? "Remove from Favourites" : "Add to Favourites"}
                aria-label={encounter.isFavorite ? "Remove from Favourites" : "Add to Favourites"}
              >
                <svg class="w-4 h-4" fill={encounter.isFavorite ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
        ...
        <tbody class="bg-background/40">
          {#each displayedPlayers as p (p.uid)}
            <tr
              ...
              onclick={() => viewPlayerSkills(p.uid, activeTab === "healing" ? "heal" : activeTab === "tanked" ? "tanked" : "dps", activeTab === "damage" ? overviewTargetUid : null)}
            >
              ...
              <TableRowGlow
                ...
                percentage={activeTab === "healing"
                  ? SETTINGS.history.general.state.relativeToTopHealPlayer && maxHealPlayer > 0
                    ? (p.healDealt / maxHealPlayer) * 100 : p.healPct
                  : activeTab === "tanked"
                    ? SETTINGS.history.general.state.relativeToTopTankedPlayer && maxTankedPlayer > 0
                      ? (p.damageTaken / maxTankedPlayer) * 100 : p.tankedPct
                    : SETTINGS.history.general.state.relativeToTopDPSPlayer && maxDpsPlayer > 0
                      ? (p.totalDmg / maxDpsPlayer) * 100 : p.dmgPct}
              />
            </tr>
          {/each}
        </tbody>
    ...
    {#if skillType === "heal"}
      <div class="mb-3 rounded border border-border/60 bg-card/30 p-3">
        <div class="text-xs uppercase tracking-wider text-muted-foreground mb-2">
          Healing Target Distribution
        </div>
        {#if healTargetSummary.length === 0}
          <div class="text-sm text-muted-foreground">No heal target data available</div>
        {:else}
            ...
            {#each healTargetSummary as target (target.targetUid)}
              ...
              <div class="text-sm">
                <div class="flex items-center justify-between gap-2 text-muted-foreground">
                  ...
                  <span class="shrink-0">{target.totalValue.toLocaleString()} ({pct.toFixed(1)}%)</span>
                </div>
            ...
            <tr class="relative border-t border-border/40 hover:bg-muted/60 transition-colors">
              <td class="px-3 py-3 text-sm text-muted-foreground relative z-10">
                {#if item.kind === "group"}
                  ...
                  >
                    <svg
                      class="size-3 shrink-0 text-muted-foreground/70 transition-transform duration-150 {expandedGroups.has(item.row.recountId) ? 'rotate-90' : ''}"
                ...
                {:else}
                  <div class="inline-flex items-center gap-1.5" style="padding-left: {item.depth * 16}px;">
                    {#if item.depth > 0}
                      <span class="w-3 shrink-0 flex justify-center"><span class="size-1 rounded-full bg-muted-foreground/35"></span></span>
                    {:else}
                      <span class="w-3 shrink-0"></span>
                    {/if}
                    <span class="truncate">{item.row.name}</span>
                    {#if item.row.showSkillId}
                      <span class="text-[10px] text-muted-foreground/50 shrink-0">#{item.row.skillId}</span>
                    {/if}
                  </div>
                {/if}
              </td>
              ...
              <TableRowGlow
                ...
                percentage={skillType === "heal"
                  ? SETTINGS.history.general.state.relativeToTopHealSkill && maxSkillTotal > 0
                    ? (rowTotalDmg(item) / maxSkillTotal) * 100 : rowDmgPct(item)
                  : skillType === "tanked"
...
{#if showDeleteModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
    <button class="absolute inset-0 bg-black/60 backdrop-blur-sm" onclick={closeDeleteModal} aria-label="Close dialog"></button>
    <div class="relative bg-card border border-border rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1">
          <h3 id="delete-modal-title" class="text-lg font-semibold text-foreground">Delete Encounter</h3>
