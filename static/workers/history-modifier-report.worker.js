(function () {
  ...
  function createFastSkillAggregate(skillId, damageIds = []) {
    return {
      skillId,
      damageIds: new Set(Array.from(damageIds).filter(Number.isFinite)),
      matches: new Set(),
      totalDmg: 0,
      effectiveTotal: 0,
      hits: 0,
  ...
  function mergeActorRef(left, right) {
    ...
    return {
      ...
      ownerUid: left.ownerUid ?? right.ownerUid,
  ...
  function entryMatchesBucket(entry, bucket, skillId, damageIds) {
    ...
    const candidates = new Set([skillId, ...damageIds]);
  ...
  function sourceMatchesForBucket(entity, catalog, ignoredBuffIds, scope, bucket, preferredId, skillId, damageIds) {
    ...
    const staticMatches = entries
      ...
      .map((entry) => ({ entry, match: "direct-static-target" }));
    ...
    return dedupeSourceMatches([
      ...
      .map((entry) => ({ entry, match: "no-static-target" })),
    ]);
  }

  function mergeEntryIntoSource(source, entry) {
    ...
    for (const hint of model?.componentValueHints ?? []) {
      ...
      for (const value of outputValues) {
        resolvedValues.push({
          ...
          predicateTags: hint.predicateTags,
  ...
  function formulaReplayContributionEstimateForSource(source, replayHits, targetUid, componentValues) {
    ...
    const componentEstimates = new Map();
    let observedTotal = 0;
  ...
  function evidenceForFormulaReplay(skill, model, componentValues) {
    ...
    if (skill.attackerAttrIds.size > 0) evidence.add("attacker stat snapshot at hit time");
    if (
    ...
    if (skill.targetAttrIds.size > 0) evidence.add("target stat snapshot at hit time");
    if (skill.targetAttrIds.has(ATTR_DEFENSE_POWER)) {
      evidence.add("target armor/defense snapshot at hit time");
    }
    if (Array.from(ELEMENTAL_RESIST_ATTR_IDS).some((attrId) => skill.targetAttrIds.has(attrId))) {
  ...
  function formulaReplayModelForSkill(skill, model, actorSummary, expectedHitCount) {
    ...
    const missingEvidence = sortedStrings([...requiredEvidence].filter((item) => !availableEvidence.has(item)));
    ...
    if (skill.mixedCritBucketCount > 0) blockers.add("at least one bucket mixes crit and non-crit hits");
    if (skill.mixedLuckyBucketCount > 0) blockers.add("at least one bucket mixes lucky and non-lucky hits");
    ...
    if (missingEvidence.length > 0) blockers.add("required formula replay evidence is missing");
  ...
  function buildModifierActivityRowsFast(entity, elapsedSecs, options) {
    ...
    for (const source of sources.values()) {
      ...
      const skillRows = Array.from(source.skills.values())
        .map((skill) => {
